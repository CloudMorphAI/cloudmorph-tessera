...
## Artifacts
Upload artifacts to Azure Blob Storage and return pointer payloads only (for example, kind `azure:blob`).
...
export CONTROL_CENTER_ACCOUNT_ID=azure-subscription-id
python src/main.py
```
...
```bash
docker build -f azure/executor/Dockerfile -t cloudmorph-executor .
```
