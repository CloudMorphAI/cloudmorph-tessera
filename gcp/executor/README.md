...
## Artifact pointers
Store artifacts in GCS and return only pointer metadata, for example:
- kind: gcs
...
```bash
docker build -f gcp/executor/Dockerfile -t cloudmorph-executor-gcp .
```
