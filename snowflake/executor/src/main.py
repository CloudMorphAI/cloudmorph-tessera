...
def main() -> None:
    ...
    missing = [name for name in ("CONTROL_CENTER_API_URL", "CONTROL_CENTER_EXECUTOR_TOKEN", "CONTROL_CENTER_TENANT_ID", "CONTROL_CENTER_ACCOUNT_ID") if not os.getenv(name)]
    if missing:
    ...
    _log(
        ...
        storageBucket=os.getenv("STORAGE_BUCKET") or os.getenv("STORAGE_LOCATION"),
        storagePrefix=os.getenv("STORAGE_PREFIX"),
        storageRegion=os.getenv("STORAGE_REGION") or os.getenv("AWS_REGION") or os.getenv("AWS_DEFAULT_REGION"),
    ...
    if oneshot_job_id and oneshot_job_token:
        ...
        try:
            ...
            try:
                _log("info", "executor.oneshot.output", jobId=oneshot_job_id, **_output_meta(status, reason, summary, logs, artifacts, result.get("result")))
    ...
    def _handle_signal(signum, frame):
        _log("info", "executor.shutdown.signal", signal=signum)
        shutdown_event.set()
    ...
    while not shutdown_event.is_set():
        ...
        try:
            summary = summary or f"Job {status}"
            _log("info", "executor.job.output", jobId=job_id, **_output_meta(status, reason, summary, logs, artifacts, result_payload))
            client.post_complete(
                job_id,
                job_token,
                status,
