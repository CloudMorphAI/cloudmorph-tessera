...
```bash
docker pull ghcr.io/cloudmorphai/tessera:0.4.0
```
...
# Step 2: Scaffold
docker run --rm -v "$PWD:/out" ghcr.io/cloudmorphai/tessera:0.4.0 tessera init --dir /out
...
docker run -d --name tessera \
  ...
  ghcr.io/cloudmorphai/tessera:0.4.0
...
Tessera v0.4.0 ships **12 reference policies** out of the box (7 generic + 5 AWS-illustrative). Drop any of them into your `--policy-dir` to opt in. All policies are simple YAML and easy to fork. Vendor-specific policies (GitHub, Jira, OWASP, Postgres, Salesforce, Slack) are in the Tessera Cloud premium pack.
